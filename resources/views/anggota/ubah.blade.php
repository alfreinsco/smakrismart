        ...
        <div class="row align-items-center">
            <div class="col-md-8">
                <div class="page-header-title">
                    <h5 class="m-b-10">Ubah Data Anggota</h5>
                    <p class="m-b-0">Ubah Data anggota perpustakaan pada aplikasi <span class="text-warning">SMAKRISMART</span></p>
                </div>
            </div>
            <div class="col-md-4">
                <ul class="breadcrumb-title">
                    ...
                    <li class="breadcrumb-item"><a href="{{route('anggota.ubah', $anggota->id)}}">Ubah</a>
                    </li>
                </ul>
            </div>
        </div>
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h5>Ubah Data Anggota Perpustakaan</h5>
                                <span>Inputan dengan tanda <code>*</code> Wajib diisi!</span>
                            </div>
                            <div class="card-block">
                                <form class="form-material" action="{{route('anggota.ubah', $anggota->id)}}" method="POST" enctype="multipart/form-data">@csrf
                                    <div class="form-group form-default">
                                        <input required onkeyup="$('#vnama').text($(this).val())"  type="text" name="nama" id="nama" class="form-control @error('nama') is-invalid @enderror" value="{{old('nama') ?? ($anggota->nama ?? '')}}">
                                    ...
                                    <div class="form-group form-default">
                                        <input required onkeyup="$('#vnomor_identitas').text($(this).val())" type="number" class="form-control @error('nomor_identitas') is-invalid @enderror" name="nomor_identitas" id="nomor_identitas" value="{{old('nomor_identitas') ?? ($anggota->nomor_identitas ?? '')}}">
                                        <span class="form-bar"></span>
                                    ...
                                    <script>
                                        $(document).ready(function(){
                                            ...
                                            $("#kodeBatangContainer").html("").show().barcode("{{$anggota->nomor_identitas}}", "code128", settings);
                                        });
                                    </script>
                                    <div class="form-group form-default">
                                        <input required type="text" class="form-control @error('nomor_telepon') is-invalid @enderror" name="nomor_telepon" id="nomor_telepon" value="{{old('nomor_telepon') ?? ($anggota->nomor_telepon ?? '')}}">
                                        <span class="form-bar"></span>
                                    ...
                                    <div class="form-group form-default">
                                        <select class="form-control @error('jabatan') is-invalid @enderror" name="jabatan" id="jabatan">
                                            <option value=""></option>
                                            <option @if(old('jabatan') == 'Siswa' || (isset($anggota) && $anggota->jabatan == 'Siswa')) selected @endif value="Siswa">Siswa</option>
                                            <option @if(old('jabatan') == 'Guru' || (isset($anggota) && $anggota->jabatan == 'Guru')) selected @endif value="Guru">Guru</option>
                                        </select>
                                        <span class="form-bar"></span>
                                    ...
                                    <div class="row">
                                        <div class="form-group form-default col-6">
                                            <input @if(old('jenis_kelamin') == 'Laki-Laki' || (isset($anggota) && $anggota->jenis_kelamin == 'Laki-Laki')) checked @endif onchange="$('#vjenis_kelamin').text('Laki-Laki')" type="radio" class="@error('jenis_kelamin') is-invalid @enderror" name="jenis_kelamin" id="jenis_kelaminl" value="Laki-Laki">
                                            <label for="jenis_kelaminl">Laki-Laki</label>
                                            <input @if(old('jenis_kelamin') == 'Perempuan' || (isset($anggota) && $anggota->jenis_kelamin == 'Perempuan')) checked @endif onchange="$('#vjenis_kelamin').text('Perempuan')" type="radio" class="@error('jenis_kelamin') is-invalid @enderror" name="jenis_kelamin" id="jenis_kelaminp" value="Perempuan">
                                        ...
                                        <div class="col-6 text-right">
                                            ...
                                            <div class="w-100 text-right">
                                                <button type="button" class="btn btn-primary btn-sm tombol_status @if(old('status') == 'Aktif' || (isset($anggota) && $anggota->status == 'Aktif')) btn-primary @else btn-danger @endif">{{ old('status', isset($anggota) ? ($anggota->status == 'Aktif' ? 'Aktif' : 'Tidak Aktif') : 'Aktif') }}</button>
                                                <input type="hidden" class="@error('status') is-invalid @enderror" role="switch" name="status" id="status" value="{{old('status', isset($anggota) ? $anggota->status : 'Aktif')}}">
                                                @error('status')
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                    ...
                                    <div class="form-group form-default">
                                        <input required onkeyup="$('#valamat').text($(this).val())" type="text" class="form-control @error('alamat') is-invalid @enderror" name="alamat" id="alamat" value="{{old('alamat') ?? ($anggota->alamat ?? '')}}">
                                        <span class="form-bar"></span>
                                    ...
                                    <div class="form-group form-default text-right">
                                        <a href="{{route('anggota.index')}}" class="btn btn-warning btn-sm"><i class="fa fa-backward"></i> Kembali</a>
                                        ...
                                        <div class="row mt-2">
                                            ...
                                            <div class="col-8">
                                                <table class="w-100" style="font-size: 10px">
                                                    <tr>
                                                        <th>Nama</th>
                                                        <td>: <span id="vnama">{{$anggota->nama}}</span></td>
                                                    </tr>
                                                    <tr>
                                                        <th>NIS/NIP</th>
                                                        <td>: <span id="vnomor_identitas">{{$anggota->nomor_identitas}}</span></td>
                                                    </tr>
                                                    <tr>
                                                        <th>Jenis Kelamin</th>
                                                        <td>: <span id="vjenis_kelamin">{{$anggota->jenis_kelamin}}</span></td>
                                                    </tr>
                                                    <tr>
                                                        <th>Alamat</th>
                                                        <td>: <span id="valamat">{{$anggota->alamat}}</span></td>
                                                    </tr>
                                                </table>
                                            </div>
                                            <div class="col-6"></div>
